...
public class JdbcStorageConfigManager {
  ...
  public static String removeDbcpPrefix(String key) {
    if (key.startsWith(JdbcStorageConfig.DBCP_CONFIG_PREFIX + ".")) {
      return key.substring(JdbcStorageConfig.DBCP_CONFIG_PREFIX.length() + 1);
    }
    return key;
  }
}
